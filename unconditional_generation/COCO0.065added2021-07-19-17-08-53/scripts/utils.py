        ...
        with open(path, 'r') as f:
            ...
            for line in f:
                ...
                if len(line.split()) > self.maxlen:
                    ...
                    continue
                # vectorize
                indices = self.bertTokenizer.convert_tokens_to_ids(self.bertTokenizer.tokenize(line))
                lines.append(indices)
...
def batchify(data, bsz, max_len, shuffle=False, gpu=False):
    ...
    for i in range(nbatch):
        ...
        for x, y in zip(source, target):
            ...
            x += zeros
            y += zeros
        source = torch.LongTensor(np.array(source).astype(np.int64))
        target = torch.LongTensor(np.array(target).astype(np.int64)).view(-1)
