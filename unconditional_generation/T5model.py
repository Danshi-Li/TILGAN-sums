...
class AE_T5(nn.Module):
    def __init__(self, add_noise, emsize, nhidden, ntokens, nlayers, nheads, nff, aehidden, noise_r=0.2,
        ...
        self.T5 = T5ForConditionalGeneration(self.config)
        ...
        self.unsqueeze_hidden = nn.Linear(self.nhidden, self.config.d_model)
        self.linear = self.T5.lm_head
    ...
    def forward(self, indices, lengths, target, add_noise, soft, encode_only=False):
        ...
        src = indices.transpose(0,1).contiguous() #[16,64] = [max_len, batchsize]
        ...
        if add_noise:
            #TODO: add noise to encoder input
            pass

        memory_bank = self.encoder(input_ids=src).last_hidden_state #enc_state=[16,64,512]  memory_back=[16,64,100] lengths=[64]
        ...
        tgt = tgt.contiguous()
        #print("forward input shape:",tgt.shape)
        dec_out = self.decoder(input_ids=tgt, encoder_hidden_states=memory_bank).last_hidden_state
        ...
        decoded = decoded.view(batchsize, max_len, self.config.vocab_size)
    ...
    def generate(self, hidden, maxlen, sample=True, temp=1.0):
        ...
        decoder_input = self.start_symbols.unsqueeze(2) #[1,64,1]
        memory_bank = hidden.view(batch_size, maxlen, -1).contiguous()  #[64,3300] -> [64,33,100] -> [33,64,100]
        #print("generate memory bank size:",memory_bank.shape)
        #print("unsqueeze hidden weight:",self.unsqueeze_hidden.weight.shape)
        ...
        for step in range(maxlen):
            ...
            decoder_input = decoder_input.squeeze(-1).contiguous()
            #print("==============================")
            #print("generate input in step {}:".format(step),decoder_input)
            dec_out= self.decoder(input_ids=decoder_input, encoder_hidden_states=memory_bank).last_hidden_state
            dec_out = dec_out    # dec_out [64,1,512] -> [64,512]
            decoded = self.linear(dec_out)   #[64,ntokens]
            #print("generate output in step {}:".format(step),dec_out)
