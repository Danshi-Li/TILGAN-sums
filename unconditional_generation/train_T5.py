...
from T5model import AE_T5, MLP_D, MLP_D_local, MLP_G
...
corpus = Corpus(args.data_path,
                ...
                T5=True)
maxlen = 0
for i in range(len(corpus.train_T5)):
    if len(corpus.train_T5[i]) > maxlen:
        maxlen = len(corpus.train_T5[i])
for i in range(len(corpus.test_T5)):
    if len(corpus.test_T5[i]) > maxlen:
        maxlen = len(corpus.test_T5)
args.maxlen = maxlen

# save arguments

ntokens = conf.vocab_size
print("Vocabulary Size: {}".format(ntokens))
args.ntokens = ntokens

# exp dir
create_exp_dir(os.path.join(args.save), ['train_T5.py', 'models.py', 'utils.py'],
...
test_data = batchify(corpus.test_T5, eval_batch_size, args.maxlen, shuffle=False, T5=True)
train_data = batchify(corpus.train_T5, args.batch_size, args.maxlen,  shuffle=True, T5=True)
...
# Build the models
###############################################################################
autoencoder = AE_T5(add_noise=args.add_noise,
                      emsize=args.emsize,
...
nlatent = args.nhidden * (args.maxlen+1)
    ...
    for i, batch in enumerate(data_source):
        ...
        with open(aeoutf, "w") as f:
            ...
            for t, idx in zip(target, max_indices):
                # real sentence
                chars = corpus.T5Tokenizer.decode(t)
                f.write(chars + '\n')
                # autoencoder output sentence
                chars = corpus.T5Tokenizer.decode(idx)
                f.write(chars + '\n'*2)
...
def gen_fixed_noise(noise, to_save):
    gan_gen.eval()
    autoencoder.eval()
    #print("fixed noise shape:",noise.shape)
    fake_hidden = gan_gen(noise)
    #print("fake hidden shape:",fake_hidden.shape)
    max_indices = autoencoder.generate(fake_hidden, maxlen+1, sample=args.sample)

    with open(to_save, "w") as f:
        max_indices = max_indices.data.cpu().numpy()
        for idx in max_indices:
            # generated sentence
            idx = torch.Tensor(idx).squeeze(1)
            words = corpus.T5Tokenizer.decode(idx)
...
def train_ae(epoch, batch, total_loss_ae, start_time, i):
    ...
    source, target, lengths = batch
    #print("source:",corpus.T5Tokenizer.decode(source[0]))
    source = Variable(source.to(device))
    target = Variable(target.to(device))
    output = autoencoder(source, lengths, source, add_noise=args.add_noise, soft=False)
    ...
    #print("decoded target:",corpus.T5Tokenizer.decode(masked_target))
