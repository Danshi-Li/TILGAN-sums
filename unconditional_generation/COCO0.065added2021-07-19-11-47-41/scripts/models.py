...
class AE_BERT_enc(nn.Module):
    def __init__(self, vocab, add_noise, emsize, nhidden, ntokens, nlayers, nheads, nff, aehidden, noise_r=0.2,
        ...
        self.dropout = dropout
        self.gpu = gpu
        self.config = BertConfig(num_hidden_layers=nlayers, hidden_size=aehidden, num_attention_heads=nheads, attention_probs_dropout_prob=dropout, hidden_dropout_prob=dropout,max_position_embeddings=aehidden)
        self.start_symbols = to_gpu(gpu, Variable(torch.ones(10, 1).long()))
